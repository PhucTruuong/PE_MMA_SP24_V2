...
const MenuScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const sortedPriceData = data.sort((a, b) => a.price - b.price);
    const filterGender = sortedPriceData.filter((item) => item.gender === false);

    if (activeCompanies !== "All") {
        filterGender = filterGender.filter((item) => item.company === activeCompanies);
    };

    if (searchTerm !== "") {
        filterGender = filterGender.filter((item) =>
        item.perfumeName.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    setFilteredAndSearchedData(filterGender);
